...
#Install all the needed packages, i.e. install.packages("pROC") etc if such package is not installed yet. Afterwards, load all the needed packages.
library(caret)
library(MASS)
library(tidyverse)
...
library(ipred)
##############################################################################
#Paper 1: Supervised Machine Learning Prediction of Non-Communicable Diseases#
##############################################################################
...
ctrl <- trainControl(method ="repeatedcv", number = 10, repeats = 5, savePredictions = "all", sampling = "down")
...
######### Logistic Regression ##########
set.seed(250303)
#ctrl.lr <- trainControl(method ="repeatedcv", number = 10, repeats = 5, savePredictions = "all", classProbs = TRUE, sampling = "down")
...
set.seed(250307)
#ctrl.lda <- trainControl(method ="repeatedcv", number = 10, repeats = 5, savePredictions = "all", classProbs = TRUE, sampling = "down")
...
######### KNN ##########
set.seed(250308)
#ctrl.knn <- trainControl(method ="repeatedcv", number = 10, repeats = 5, savePredictions = "all", classProbs = TRUE, sampling = "down")
...
######### Random Forest ##########
set.seed(250309)
#ctrl.rf <- trainControl(method ="repeatedcv", number = 10, repeats = 5, savePredictions = "all", classProbs = TRUE, sampling = "down")
...
######### Bagging ##########
set.seed(250310)
initial.bag <- bagging(overweight ~ ., data = train.data, nbagg = 100, coob = TRUE,control = rpart.control(minsplit = 2, cp = 0))

ctrl.bag <- trainControl(method ="repeatedcv", number = 10, repeats = 5, savePredictions = "all",  sampling = "down")
...
model.boost <- gbm(overweight ~ ., data = btrain.data, n.trees = 500, distribution = "bernoulli", interaction.depth = 1, cv.folds = 50, shrinkage = 0.2)
...
#######################Metric Analysis#####################
#############ACURRACY ANALYSIS ###############
acc = c(0.6468, 0.6567, 0.6119, 0.5672, 0.6269, 0.6816, 0.6716, 0.5920, 0.6119, 0.7861)
models = c("SVM", "LR", "MARS", "NB", "Tree", "LDA", "KNN", "RF", "Bagging","GB")
#par(mfrow = c(1,2))

plot(acc, xaxt = "n", xlab = "Algorithms", type = "b", ylab = "Accuracy", col = 3, lwd = 5)
axis(side = 1, at = seq(1,10), labels = models)

###############################################

############# BALANCED ACCURACY ANALYSIS ###############
bacc = c(0.6511, 0.6216, 0.5730, 0.6092, 0.6255, 0.6808, 0.6741, 0.5716, 0.5851, 0.7684)
models = c("SVM", "LR", "MARS", "NB", "Tree", "LDA", "KNN", "RF", "Bagging","GB")
plot(bacc, xaxt = "n", xlab = "Algorithms", type = "b", ylab = "Balanced Accuracy", col = 4, lwd = 5)
axis(side = 1, at = seq(1,10), labels = models)

#############SENSITIVITY ANALYSIS ###############
sen = c(0.6604, 0.5472, 0.4906, 0.6981, 0.6226, 0.6792, 0.6792, 0.5283, 0.5283, 0.7868)
models = c("SVM", "LR", "MARS", "NB", "Tree", "LDA", "KNN", "RF", "Bagging","GB")
...
############# F1 Score ANALYSIS ###############
f1 = c(0.4965, 0.4567, 0.4000, 0.4596, 0.4681, 0.5294, 0.5217, 0.4058, 0.4179, 0.8782)
models = c("SVM", "LR", "MARS", "NB", "Tree", "LDA", "KNN", "RF", "Bagging","GB")
plot(f1, xaxt = "n", xlab = "Algorithms", type = "b", ylab = "F1 Score", col = 5, lwd = 5)
axis(side = 1, at = seq(1,10), labels = models)
###############################################